...
func init() {
	...
	stockList := []string{"DOMO", "TLRY", "SQ", "MRO", "AAPL", "GM", "SNAP",
		"SHOP", "SPLK", "BA", "AMZN", "SUI", "SUN", "TSLA",
		"CGC", "SPWR", "NIO", "CAT", "MSFT", "PANW", "OKTA",
		"TWTR", "TM", "RTN", "ATVI", "GS", "BAC", "MS",
		"TWLO", "QCOM"}
...
func main() {
	...
	if account.TradingBlocked {
		log.Println("Account is currently restricted from trading.")
	}
	...
	for {
		var timeToOpen float64
		isOpen := alpacaClient.awaitMarketOpen(&timeToOpen)
		if isOpen {
			break
		}
		time.Sleep(5 * time.Minute)
	}
	log.Println("Market Opened.")
...
func (alp alpacaClientContainer) run() {
	...
	if clock.NextClose.Sub(clock.Timestamp) < 15*time.Minute {
		...
		log.Println("Market closing soon.  Closing positions.")
		...
		for _, position := range positions {
			...
			qty = math.Abs(qty)
			alpacaClient.submitOrder(qty, position.Symbol, orderSide)
		}
		// Run script again after market close for next trading day.
		log.Println("Sleeping until market close (15 minutes).")
...
func (alp alpacaClientContainer) awaitMarketOpen(timeToOpen *float64) bool {
	clock, _ := alpacaClient.client.GetClock()
	if clock.IsOpen {
		return true
	}
	*timeToOpen = clock.NextOpen.Sub(clock.Timestamp).Minutes()
	log.Println(time.Duration(*timeToOpen)*time.Minute, " minutes until next market open.")
	return false
}
...
func (alp alpacaClientContainer) rebalance() {
	...
	log.Printf("We are taking a long position in: %v.\n", alpacaClient.long.list)
	log.Printf("We are taking a short position in: %v.\n", alpacaClient.short.list)

	log.Print("We are taking a long position in: ")
	log.Printf("%v", alpacaClient.long.list)
	log.Print("We are taking a short position in: ")
	log.Printf("%v", alpacaClient.short.list)
	...
	if len(longBOResp[1][:]) > 0 {
		...
		if longTPResp > 0 {
			alpacaClient.long.adjustedQty = alpacaClient.long.equityAmt / longTPResp
		} else {
	...
	if len(shortBOResp[1][:]) > 0 {
		...
		if shortTPResp > 0 {
			alpacaClient.short.adjustedQty = alpacaClient.short.equityAmt / shortTPResp
		} else {
...
func (alp alpacaClientContainer) rerank() {
	...
	alpacaClient.long.qty = alpacaClient.long.equityAmt / longTotal
	alpacaClient.short.qty = alpacaClient.short.equityAmt / shortTotal
}

// Get the total price of the array of input stocks.
func (alp alpacaClientContainer) getTotalPrice(arr []string) float64 {
	totalPrice := 0.0
	log.Println("getTotalPrice()")
...
func (alp alpacaClientContainer) submitOrder(qty float64, symbol string, side string) error {
	account, _ := alpacaClient.client.GetAccount()
	if qty > 0 && qty < math.Inf(1) {
		...
		if err == nil {
			log.Println("Market order of | ", qty, " ", symbol, " ", side, " | completed.")
		} else {
			log.Println("Order of | ", qty, " ", symbol, " ", side, " | did not go through.")
			log.Println("error: ", err)
			roundedQty := int(qty)
			_, err := alpacaClient.client.PlaceOrder(alpaca.PlaceOrderRequest{
				AccountID:   account.ID,
				AssetKey:    &symbol,
				Qty:         decimal.NewFromFloat(float64(roundedQty)),
				Side:        adjSide,
				Type:        "market",
				TimeInForce: "day",
			})
			if err == nil {
				log.Println("Market order of | ", roundedQty, " ", symbol, " ", side, " | completed.")
			} else {
				log.Println("error: ", err)
			}
		}
	...
	log.Println("Quantity is <= 0, order of | ", qty, " ", symbol, " ", side, " | not sent.")
	return nil
}

// Submit a batch order that returns completed and uncompleted orders.
func (alp alpacaClientContainer) sendBatchOrder(qty float64, stocks []string, side string) [2][]string {
	var executed []string
...
func (alp alpacaClientContainer) getPercentChanges() {
	log.Println("range alpacaClient.allStocks", len(alpacaClient.allStocks))
	length := 10
	for i, stock := range alpacaClient.allStocks {
		log.Println("i: ", i, " stock: ", stock)
